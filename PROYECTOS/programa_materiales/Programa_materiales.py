#Este scrip fue ehcho porr Diego Suarez Ladero, lo hice solo ya que no pude enconmtrar pareja.
...
nombres=[]
tamaños=[]
opcion = 0 #\Users\Pc\Documents\UNI\2º-Cuatri\Fundamentos de InformáStica\RepositorioFundamentosInformatica\PROYECTOS\programa materiales\materiales.csv
continua = True
while continua:
    ...
    try:
        ...
        print('-'*30)
        if opcion == 1:
            abrir_archivo(input('Selecciona archivo (usa ruta absoluta): '))
        elif opcion == 2:
            if len(nombres) == 0: 
                print('EEROR : Debe cargar un archivo primero')
                print('-'*30) 
            else:
                listar_materiales(nombres,tamaños)  
        elif opcion == 3:
            if len(nombres) == 0: 
                print('EEROR : Debe cargar un archivo primero')
                print('-'*30) 
